...
export class RabbitConsumerAdapter implements IMessagingConsumerAdapter {
  ...
  protected async consume() {
    ...
    const { channel, connection } = await this.amqpConnection({ exchange, queues: [queueName] })
  ...
  private async consumeArray(channel: Channel, queueName: string, queueMessage: any) {
